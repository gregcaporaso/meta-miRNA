...
# The human miRBase database needs to be downloaded by the user.
    ...
    def run(self, **kwargs):
        ...
        for input_filepath in input_filepaths:
            ...
            command = "%s %s -r %s -o %s --enable_rev_strand_match --max_accepts 1 --max_rejects 8 --stepwords 8 --word_length 8 -O 20" % (self.parallel_pick_otus_uclust_ref_path, input_filepath, all_ncrna_except_miRNA_database_dir, temp_dir)
            if self.verbose:
                print command
            ...
            stdout, stderr, ret_val = pyqi_system_call(command)
            command = "%s %s -r %s -o %s --enable_rev_strand_match --max_accepts 1 --max_rejects 8 --stepwords 8 --word_length 8 -O 20" % (self.parallel_pick_otus_uclust_ref_path, temp_fasta_filtered_fp, mirBase, temp_index_of_otus_hitting_miRbase_fp)
            stdout, stderr, ret_val = pyqi_system_call(command)
            if self.verbose:
                print command
