...
class FastaToParallelPickOtusUclustRef(Command):
    BriefDescription = "This script allows to run a closed-reference otu picking in Qiime using a fasta file containing miRNA sequences (i.e. output from sra_to_qiime.py script)"
    LongDescription = "A script for running closed-reference otu picking in Qiime. THIS CODE IS CURRENTLY UNTESTED. YOU SHOULD NOT USE THIS VERSION OF THE CODE. THIS MESSAGE WILL BE REMOVED WHEN TESTS ARE ADDED."
...
# Qiime is required to be installed by the User so that every scripts can be called in the command line within the User $HOME.
    ...
    qiime_path = "parallel_pick_otus_uclust_ref.py"
    ...
    def run(self, **kwargs):
        ...
        for input_filepath in input_filepaths:
            ...
            # create and call the parallel_pick_otus_uclust_ref.py command
            temp_fastq_dir = join(self.temp_dir, '%s_fastq' % input_basename) 
            command = "%s %s -O %s" % (self.qiime_path, input_filepath, temp_fastq_dir)
            if self.verbose:
                print command
